...
if __name__ == '__main__':
    fTyp = [("csvファイル", "*.csv")]        #入力の指定
    iDir = os.path.abspath(os.path.dirname(__file__))      #ダイアログの初期値を指定
    #__file__は実行中のファイル、abspathは絶対パス->実行中ファイルがあるディレクトリの絶対パス
    a = filedialog.askopenfilename(filetypes=fTyp, initialdir=iDir)
    a = os.path.basename(a)
    #苦し紛れの導入（後のファイル名命名部分を変えてこの文をなくしたい）
